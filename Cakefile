...
task "build", "compile all coffeescript files to javascript", ->
  ...
  coffee.on 'exit', (returnCode) ->
    process.exit returnCode if returnCode != 0

    # Build pages/vimium_resources.html from the Vimium resources listed in lib/inject_vimium.coffee.
    {jss, css} = require "./lib/inject_vimium.js"
    jss.unshift "/lib/inject_vimium.js"
    lines = []
    lines.push "<!-- This file is automatically generated by 'cake build'; do not edit it directly. -->"
    lines.push ""
    lines.push "<script src=\"#{js}\"></script>" for js in jss
    lines.push ""
    lines.push "<link rel=\"stylesheet\" type=\"text/css\" href=\"#{cs}\" />" for cs in css
    fs.writeFile "./pages/vimium_resources.html", "#{lines.join "\n"}\n", (err) ->
      if err
        console.error "#{err}"
        process.exit 1
