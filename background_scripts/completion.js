...
class SearchEngineCompleter {
  ...
  filter(request, onComplete) {
    ...
    // This filter is applied to all of the suggestions from all of the completers, after they have
    // been aggregated by the MultiCompleter.
    ...
    // If a previous suggestion still matches the query, then we keep it (even if the completion
    // engine may not return it for the current query). This allows the user to pick suggestions
    // that they've previously seen by typing fragments of their text, without regard to whether the
    // completion engine can continue to complete the actual text of the query.
    ...
    const cachedSuggestions = haveCompletionEngine
      ? CompletionSearch.complete(searchUrl, queryTerms)
      : null;
