...
class Movement
  ...
  runMovement: (args...) ->
    ...
    if granularity == vimword and direction == forward
      while @nextCharacterIsWordCharacter()
        unless @runMovements [forward, character]
          return
  ...
  reverseSelection: ->
    ...
    if element and DomUtils.isEditable(element) and not element.isContentEditable
      ...
      for [0...length] by 1
        @runMovement @opposite[direction], character
    else
      # Normal method.
...
class VisualMode extends KeyHandlerMode
  init: (options = {}) ->
    ...
    @onExit (event = null) =>
      ...
      else if event and event.type == "keydown" and KeyboardUtils.isEscape(event) and @name != "caret"
    ...
    unless @name == "caret"
      if ["Caret", "Range"].includes(DomUtils.getSelectionType(@selection))
