...
insertModeLock = null
findMode = false
findModeQuery = { rawQuery: "", matches: [] }
...
sameRange = (first, second) ->
  first.compareBoundaryPoints(Range.START_TO_START, second) is 0 and
    first.compareBoundaryPoints(Range.END_TO_END, second) is 0

updateMatches = (query, options) ->
  cachedScrollX = window.scrollX
  cachedScrollY = window.scrollY
  currentMatch = window.getSelection().getRangeAt(0)
  findModeQuery.matches = []

  while (window.find(query, options.caseSensitive, options.backwards, true, false, true, false))
    range = window.getSelection().getRangeAt(0)
    findModeQuery.matches.push(range)
    break if sameRange(currentMatch, range)
...
executeFind = (query, options) ->
  ...
  updateMatches(query, options) if (result)
...
showFindModeHUDForQuery = ->
  if (findModeQueryHasResults || findModeQuery.parsedQuery.length == 0)
    HUD.show("/" + findModeQuery.rawQuery + " (" + findModeQuery.matches.length + " matches)")
  else
...
window.showHelpDialog = (html, fid) ->
  ...
  VimiumHelpDialog =
    ...
    init: () ->
      ...
      @dialogElement.style.maxHeight = window.innerHeight - 80
