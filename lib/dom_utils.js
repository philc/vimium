var DomUtils = {
  ...
  // Note that adding these nodes all at once (via the parent div) is significantly faster than
  // one-by-one.
  //
  ...
  simulateClick(element, modifiers) {
    ...
    for (let event of eventSequence) {
      ...
      // While 91.6.0esr also merged the patch, so it's necessary to check the minor version
      // (#4000).
      // The root difference is a logic change imported in
      // https://bugzilla.mozilla.org/show_bug.cgi?id=1739929 .
      let mayBeBlocked = false;
      if (event === "click" && Utils.isFirefox()) {
        const [major, minor] = (Utils.firefoxVersion() + "").split(".").map((i) =>
          parseInt(i) || 0
        );
        if (major > 0 && major < 96) {
          if (major !== 91) {
            ...
            element.target === "_blank" && Object.keys(modifiers).length === 0 ||
            minor != null && minor < 6
          ) {
      ...
      const defaultActionShouldTrigger = mayBeBlocked && (Object.keys(modifiers).length === 0) &&
  ...
  simulateMouseEvent(event, element, modifiers) {
    ...
    // Debugging note: Firefox will not execute the element's default action if we dispatch this
    // click event, but Webkit will. Dispatching a click on an input box does not seem to focus it;
  ...
  injectUserCss() {
    ...
    style.type = "text/css";
    style.textContent = Settings2.get("userDefinedLinkHintCss");
