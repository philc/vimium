const DomUtils = {
  ...
  _documentReadyCallbacks: [],

  documentReady(callback) {
    if (this.isReady()) {
      // TODO(philc): I think we can/should remove this return value. Nothing should be relying on
      // it.
      return callback();
    } else if (this._documentReadyCallbacks.length > 0) {
      this._documentReadyCallbacks.push(callback);
    } else {
      // The first time we enqueue a documentReady callback, add a listener for DOMContentLoaded.
      this._documentReadyCallbacks.push(callback);
      const onDomReady = forTrusted(() => {
        globalThis.removeEventListener("DOMContentLoaded", onDomReady, true);
        while (this._documentReadyCallbacks.length > 0) {
          const callback = this._documentReadyCallbacks.shift();
          callback();
        }
      });
      globalThis.addEventListener("DOMContentLoaded", onDomReady, true);
    }
  },

  _documentCompleteCallbacks: [],

  documentComplete(callback) {
    const isComplete = document.readyState === "complete";
    if (isComplete) {
      callback();
    } else if (this._documentCompleteCallbacks.length > 0) {
      this._documentCompleteCallbacks.push(callback);
    } else {
      // The first time we enqueue a documentComplete callback, add a listener for "load".
      const onLoad = forTrusted((event) => {
        ...
        while (this._documentCompleteCallbacks.length > 0) {
          const callback = this._documentCompleteCallbacks.shift();
          callback();
        }
      });
