// NOTE(mrmr1993): This is under lib/ since it is used by both content scripts and iframes from
// pages/.
// This implements find-mode query history (using the "findModeRawQueryList" setting) as a list of
// raw queries, most recent first.
const FindModeHistory = {
  // Guard against chrome being undefined (in the HUD iframe).
  // TODO(philc): How is this possible? Can we remove this check?
  storage: (typeof chrome !== "undefined" && chrome !== null ? chrome.storage.session : undefined),
  key: "findModeRawQueryList",
  max: 50,
  rawQueryList: null,

  init() {
    this.isIncognitoMode = typeof chrome !== "undefined" && chrome !== null
      ? chrome.extension.inIncognitoContext
      : undefined;

    // chrome is undefined in the HUD iframe during tests, so we do nothing.
    if (this.isIncognitoMode == null) return;
    ...
    if (!this.rawQueryList) {
      ...
      this.storage.get(this.key, (items) => {
        if (!chrome.runtime.lastError) {
          if (items[this.key]) {
            this.rawQueryList = items[this.key] || [];
          }
          ...
          if (!chrome.runtime.lastError && items.findModeRawQueryListIncognito) {
            this.storage.set({
              findModeRawQueryListIncognito: this.refreshRawQueryList(
                query,
                items.findModeRawQueryListIncognito,
              ),
            });
          }
        });
