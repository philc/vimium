// A "setting" is a stored key/value pair. An "option" is a setting which has a default value and
// whose value can be changed on the options page.
//
...
// 2. cached in Settings.cache; on extension pages, Settings.cache uses localStorage (so it
// persists).
//
...
// We use "try" because this fails within iframes on Firefox (where failure doesn't actually
// matter).
// TODO(philc): Is this still needed?
try {
  if (globalThis.chrome == null) {
    globalThis.chrome = globalThis.top != null ? globalThis.top.chrome : undefined;
  }
} catch (error) {}
...
const Settings = {
  ...
  init() {
    if (Utils.isExtensionPage() && Utils.isExtensionPage(globalThis.top)) {
      // On extension pages, we use localStorage (or a copy of it) as the cache.
      // For UIComponents (or other content of ours in an iframe within a regular page), we can't
      // access localStorage, so we check that the top level frame is also an extension page.
    ...
    // NOTE(mrmr1993, 2017-04-18): currently the API is defined in FF, but it is disabled behind a
    // flag in about:config. Every use sets chrome.runtime.lastError, so we use that to check
    ...
    chrome.storage.sync.get(null, () => {
      ...
      // Delay this initialisation until after the correct storage area is known. The significance
      // of this is that it delays the on-loaded callbacks.
  ...
  // Called after @cache has been initialized. On extension pages, this will be called twice, but
  // that does not matter because it's idempotent.
  ...
  // TODO(philc): This return value behavior is strange. Ideally this returns nil, as you would
  // expect from a potentially async function.
  onLoaded(callback) {
    if (this.isLoaded) {
      return callback();
    } else {
  ...
  handleUpdateFromChromeStorage(key, value) {
    ...
    // Note: value here is either null or a JSONified string. Therefore, even falsy settings values
    // (like false, 0 or "") are truthy here. Only null is falsy.
  ...
  set(key, value, shouldSetInSyncedStorage) {
    ...
    this.log(
      ...
      }, shouldSetInSyncedStorage=${shouldSetInSyncedStorage})`,
    );
    if (this.shouldSyncKey(key)) {
    ...
    // Any postUpdateHooks that can be called from the options page/exclusions popup should be
    // careful not to use |value| asynchronously, or else it may refer to a |DeadObject| and
    // accesses will throw an error.
  ...
  // Completely remove a settings value, e.g. after migration to a new setting. This should probably
  // only be called from the background page.
  nuke(key) {
