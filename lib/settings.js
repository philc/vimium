...
const Settings = {
  ...
  async chromeStorageOnChanged(changes) {
    // We store data with keys [settings-v1, vimiumSecret, installDate, ...] into the local
    // storage. Only broadcast an event if the object stored with the settings key has changed.
    if (changes[this.settingsKey] != null) {
      await this.load();
  ...
  async load() {
    ...
    if (!this._chromeStorageListenerInstalled) {
      ...
      chrome.storage.onChanged.addListener((changes) => this.chromeStorageOnChanged(changes));
    }
