...
// Note(gdh1995): Info in navigator is not reliable, because sometimes browsers will provide fake
// values. For example, when `privacy.resistFingerprinting` is enabled on `about:config` of Firefox.
let browserInfo = null;
if (globalThis.browser && browser.runtime && browser.runtime.getBrowserInfo) {
...
var Utils = {
  ...
  // Returns true whenever the current page (or the page supplied as an argument) is from the
  // extension's origin (and thus can access the extension's localStorage).
  isExtensionPage(win) {
  ...
  isBackgroundPage() {
    // NOTE(philc): chrome.extension.getBackgroundPage is undefined under some circumstances, but I
    // wasn't able to determine precisely which.
  ...
  createSearchUrl(query, searchUrl) {
    if (searchUrl == null) searchUrl = Settings2.get("searchUrl");
  ...
  convertToUrl(string) {
    ...
    if (Utils.hasChromePrefix(string)) {
      ...
      // In Chrome versions older than 46.0.2467.2, encoded javascript URIs weren't handled
      // correctly.
...
// This creates a new function out of an existing function, where the new function takes fewer
// arguments. This allows us to pass around functions instead of functions + a partial list of
// arguments.
...
// A simple cache. Entries used within two expiry periods are retained, otherwise they are
// discarded. At most 2 * @entries entries are retained.
class SimpleCache {
  // - expiry: expiry time in milliseconds (default, one hour)
  // - entries: maximum number of entries in @cache (there may be up to this many entries
  //   in @previous, too)
  ...
  rotate(force) {
    ...
    Utils.nextTick(() => {
      if (
        force || (this.entries < Object.keys(this.cache).length) ||
...
Object.assign(globalThis, { Utils, SimpleCache, AsyncDataFetcher, JobRunner });
