  ...
  assertType(schema, o) {
    ...
    for (const key of Object.keys(o)) {
      if (!Object.hasOwn(schema, key)) {
        throw new TypeError(`Object has unexpected property ${key}: ${o}`);
      }
      const _type = schema[key];
      if (_type == null) continue; // There is no assertion on the actual type.
...
const UserSearchEngines = {
  ...
  set(searchEnginesConfigText) {
    this.keywordToEngine = this.parseConfig(searchEnginesConfigText).keywordToEngine;
  },
};
