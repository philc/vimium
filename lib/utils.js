var utils = {
  ...
  ensureUrl: function(str) {
    ...
    // definitely an URL
    if (/^(http|https|ftp|chrome):\/\//.test(str))
      return str;
    // very likely an incomplete URL
    if (tldRegex.test(str))
      return "http://" + str;

    // fallback: we couldn't categorize this as an URL,
    // so it's probably not an URL
    return utils.createSearchUrl(str);
  },
