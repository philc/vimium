var utils = {
  ...
  /**
   * Adds scheme to :url, if none or only // is specified
   */
  createFullUrl: function(partialUrl) {
    if (/^\/\//.test(partialUrl))
      return 'http:' + partialUrl;
    else if (!/^[a-z]{3,}:\/\//.test(partialUrl))
      return 'http://' + partialUrl;
    else
      return partialUrl;
  },
  ...
  ensureUrl: (function() {
    ...
    var urlRegex = new RegExp(
      ...
      '([^:]+|\\[[^\\]]+\\])'       +   // host name (IPv6 addresses in square
                                        //            brackets allowed) => \3
    ...
    return function(str) {
      ...
      // starts with scheme or two leading slashes.
      // this can be used to force treating as an URL.
      if (/^([a-z]{3,}:)?\/\//.test(str))
      ...
      // assuming that this is an URL, try to parse it into its meaningful parts.
      // If matching fails, we're pretty sure that we don't have some kind of
      // URL here.
      ...
      // allow IPv6 addresses (need to be wrapped in brackets, as required by RFC).
      // It is sufficient to check for a colon here, as the regex wouldn't
      ...
      if (dottedParts.length > 1 && (  lastPart.length <= 3
