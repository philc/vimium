...
context "Mode constructor",
  should "Register new modes on constructor object", ->
    testMode1 = new Mode {name: "test1"}
    testMode2 = new Mode {name: "test2"}
    testMode3 = new Mode {name: "test3"}
  ...
  context "Child modes",
    should "Register a child mode from the parent", ->
      class TestParentMode extends Mode
        constructor: (name, childName) ->
          super {name}
          childMode = new Mode {name: childName, parent: this}
    ...
    should "Register a child mode via options", ->
      testModeParent = new Mode {name: "testParent"}
      testModeChild = new Mode {name: "testChild", parent: testModeParent}

      assert.equal testModeParent.modes.testChild, testModeChild

    should "Register a child mode via Mode.setMode", ->
      testModeParent = new Mode {name: "testParent"}
      testModeChild = new Mode {name: "testChild", noParent: true}
    ...
    should "Retrieve a child mode via Mode.getMode", ->
      testModeParent = new Mode {name: "testParent"}
      testModeChild1 = new Mode {name: "testChild", parent: testModeParent}
      testModeChild2 = new Mode {name: "testChild", parent: testModeChild1}
